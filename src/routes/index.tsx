...
function Index() {
  ...
  const runScan = async (target: PlaceSuggestion, cats: CategoryKey[]) => {
    ...
    const bb = target.boundingBox; const half = MAX_SPAN / 2;
    const area = { south: Math.max(bb?.south ?? -90, target.lat - half), north: Math.min(bb?.north ?? 90, target.lat + half), west: Math.max(bb?.west ?? -180, target.lon - half), east: Math.min(bb?.east ?? 180, target.lon + half) };
    try {
  ...
  return <div className="flex h-screen min-h-0 flex-col overflow-hidden bg-background text-foreground">
    <header className="relative z-[3000] flex min-h-14 shrink-0 items-center gap-2 overflow-visible border-b border-border bg-card/95 px-3 py-2 shadow-sm backdrop-blur sm:gap-3">
      ...
      <div className="min-w-0 flex-1"><PlaceSearchBar onPick={handlePickPlace} scanning={scanning} currentLabel={place?.shortLabel ?? null} /></div>
      <div className="flex shrink-0 items-center gap-1.5">
        <CategoryMenu value={categories} onChange={handleCategoriesChange} onScan={handleScanCurrentPlace} scanning={scanning} />
        <FiltersMenu ratingFilter={ratingFilter} onRatingFilterChange={setRatingFilter} priceFilter={priceFilter} onPriceFilterChange={setPriceFilter} sortKey={sortKey} onSortKeyChange={setSortKey} />
    ...
    <div className="relative z-0 flex min-h-0 flex-1 flex-col lg:flex-row lg:gap-3 lg:p-3">
      ...
      <main className="relative z-0 min-h-0 flex-1 overflow-hidden border-t border-border lg:rounded-xl lg:border lg:shadow-lg"><ClientOnly fallback={<MapSkeleton />}><Suspense fallback={<MapSkeleton />}><MapCanvas places={visibleResults} selectedId={selectedId} onSelect={setSelectedId} center={center} /></Suspense></ClientOnly>{scanning && <div className="pointer-events-none absolute left-1/2 top-3 z-[500] flex -translate-x-1/2 items-center gap-2 rounded-full border border-border bg-card/95 px-3 py-1.5 text-[11px] text-muted-foreground shadow-lg"><Loader2 className="h-3.5 w-3.5 animate-spin text-primary" />Varrendo Sinais Zero...</div>}</main>
    </div>
  </div>;
}
