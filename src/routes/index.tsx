...
import { searchOverpassServer, searchPlacesServer, verifyLeadsServer, type PlaceSuggestion } from "@/lib/geo.functions";
...
export const Route = createFileRoute("/")({ head: () => ({ meta: [{ title: "Sinal Zero — Mapa de negócios sem presença digital" }, { name: "description", content: "Encontre negócios com sinal zero de presença digital para prospecção." }] }), component: Index });
...
function Index() {
  ...
  const runScan = async (target: PlaceSuggestion, cats: CategoryKey[]) => {
    ...
    try {
      const data = await searchOverpassServer({ data: { area, categories: cats } }); if (scanId !== scanIdRef.current) return;
      const processed = processOverpassResults(data.elements, cats).filter(isStrictSignalZero).filter((r) => Boolean(r.contact.whatsappValid || r.contact.phoneDigits));
      if (processed.length === 0) { setResults([]); setVerificationMode("local"); setError("Nenhum candidato Sinal Zero com contato acionável foi encontrado nessa área."); return; }
      setScanning(true);
      const verified = await verifyLeadsServer({ data: { leads: processed.slice(0, 40) } }); if (scanId !== scanIdRef.current) return;
      setVerificationMode(verified.external ? "external" : "local");
      const finalLeads = verified.external ? verified.leads.filter((lead) => lead.verification.status === "verified" && lead.verification.score >= 85) : processed;
  ...
  const visibleResults = useMemo(() => {
    let list = results.filter(isStrictSignalZero).filter((r) => Boolean(r.contact.whatsappValid || r.contact.phoneDigits));
  ...
  return <div className="flex h-screen min-h-0 flex-col overflow-hidden bg-background text-foreground">
    <header className="relative z-[3000] flex min-h-14 shrink-0 items-center gap-2 overflow-visible border-b border-border bg-card/95 px-3 py-2 shadow-sm backdrop-blur sm:gap-3"><div className="flex shrink-0 items-center gap-2"><Radar className="h-5 w-5 text-signal-zero" /><span className="hidden text-sm font-bold tracking-tight sm:inline">Sinal <span className="text-gradient-signal">Zero</span></span></div><div className="min-w-0 flex-1"><PlaceSearchBar onPick={handlePickPlace} scanning={scanning} currentLabel={place?.shortLabel ?? null} /></div><div className="flex shrink-0 items-center gap-1.5"><CategoryMenu value={categories} onChange={handleCategoriesChange} onScan={handleScanCurrentPlace} scanning={scanning} /><FiltersMenu ratingFilter={ratingFilter} onRatingFilterChange={setRatingFilter} priceFilter={priceFilter} onPriceFilterChange={setPriceFilter} sortKey={sortKey} onSortKeyChange={setSortKey} /><div className="hidden items-center gap-2 lg:flex"><SavedLeadsDrawer leads={savedLeads} onRemove={(id) => { removeLead(id); setSavedLeads(getSavedLeads()); }} /><ExportCsvButton /></div></div></header>
    <div className="relative z-0 flex min-h-0 flex-1 flex-col overflow-hidden lg:flex-row lg:gap-3 lg:p-3"><aside className="relative z-20 flex h-[42%] min-h-0 w-full shrink-0 flex-col overflow-hidden border-b border-border bg-card/60 lg:h-auto lg:w-[460px] lg:rounded-xl lg:border"><div className="flex shrink-0 items-center justify-between border-b border-border/60 px-4 py-2.5"><div><h2 className="text-sm font-semibold">Sinais Zero</h2><p className="text-[10px] text-muted-foreground">{verificationMode === "external" ? "Verificados externamente · sem presença digital encontrada" : "Candidatos locais · verificação externa não configurada"}</p></div><span className="text-[11px] text-muted-foreground">{visibleResults.length} encontrados</span></div><div className="min-h-0 flex-1 overflow-y-auto">{scanning ? <div className="space-y-2 px-4 py-3"><div className="flex items-center gap-2 py-2 text-xs text-muted-foreground"><Loader2 className="h-3.5 w-3.5 animate-spin" />{verificationMode === "external" ? "Verificando presença digital..." : "Consultando sinais zero..."}</div>{Array.from({ length: 5 }).map((_, i) => <div key={i} className="h-16 animate-pulse rounded-md border border-border/50 bg-muted/40" />)}</div> : error ? <p className="px-4 py-6 text-xs text-destructive">{error}</p> : visibleResults.length === 0 ? <p className="px-4 py-6 text-xs text-muted-foreground">Pesquise um local, escolha as categorias e clique em Varrer área.</p> : visibleResults.map((item) => <PlaceRow key={item.id} place={item} active={item.id === selectedId} saved={savedLeads.some((l) => l.id === item.id)} onSelect={setSelectedId} onToggleSave={handleToggleSave} />)}</div></aside><main className="relative z-0 min-h-0 flex-1 overflow-hidden border-t border-border lg:rounded-xl lg:border lg:shadow-lg"><ClientOnly fallback={<MapSkeleton />}><Suspense fallback={<MapSkeleton />}><MapCanvas places={visibleResults} selectedId={selectedId} onSelect={setSelectedId} center={center} /></Suspense></ClientOnly>{scanning && <div className="pointer-events-none absolute left-1/2 top-3 z-[500] flex -translate-x-1/2 items-center gap-2 rounded-full border border-border bg-card/95 px-3 py-1.5 text-[11px] text-muted-foreground shadow-lg"><Loader2 className="h-3.5 w-3.5 animate-spin text-primary" />Verificando leads...</div>}</main></div>
  </div>;
}
