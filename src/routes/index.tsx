...
function Index() {
  ...
  const runSignalZeroVerification = async (leads: Establishment[], scanId: number) => {
    ...
    if (scanId === scanIdRef.current) setVerificationMode("local");
  ...
  const runScan = async (target: PlaceSuggestion, cats: CategoryKey[]) => {
    ...
    try {
      ...
      if (signalZeroOnly) {
        await runSignalZeroVerification(processed, scanId);
      } else {
        ...
        <aside className="relative z-20 flex h-[42%] min-h-0 w-full shrink-0 flex-col overflow-hidden border-b border-border bg-card/60 lg:h-auto lg:w-[460px] lg:rounded-xl lg:border">
          ...
          <div className="min-h-0 flex-1 overflow-y-auto">
            {scanning ? (
              ...
              <p className="px-4 py-6 text-xs text-muted-foreground">Pesquise um local, escolha as categorias e clique em Varrer área.</p>
            ) : (
