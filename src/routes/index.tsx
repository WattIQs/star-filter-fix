...
export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Mapa gratuito de prospecção: encontre restaurantes, barbearias, pet shops, lojas e mercados sem site ou Instagram, com filtros de nota, preço e contato por WhatsApp." },
      { property: "og:title", content: "Sinal Zero — Mapa de negócios sem presença digital" },
      { property: "og:description", content: "Mapa gratuito de prospecção com filtros de nota, preço e categoria, e contato direto por WhatsApp ou Instagram." },
...
function ratingMatchesFilter(rating: number | null, filter: string): boolean {
  ...
  const exactStars = Number.parseInt(filter, 10);
  if (!Number.isFinite(exactStars)) return true;
  // O filtro por estrelas representa a faixa inteira da estrela escolhida,
  // sem arredondar 4.4 para 4 ou 4.6 para 5.
  const lower = exactStars;
  const upper = exactStars === 5 ? 5 : exactStars + 1;
  return rating >= lower && (exactStars === 5 ? rating <= upper : rating < upper);
}
...
function Index() {
  ...
  const runScan = async (target: PlaceSuggestion, cats: CategoryKey[]) => {
    if (cats.length === 0) { setError("Escolha pelo menos uma categoria no menu Categorias."); return; }
    setError(null); setScanning(true); setResults([]); setSelectedId(null); setCenter({ lat: target.lat, lon: target.lon });
    try {
      ...
      if (processed.length === 0) setError("Nenhum estabelecimento encontrado aqui. Tente outra categoria ou outro local.");
    } catch (err) { setError(err instanceof Error ? err.message : "Erro ao escanear área."); }
    finally { setScanning(false); }
  };

  const handlePickPlace = (target: PlaceSuggestion) => { setPlace(target); void runScan(target, categories); };
  const handleCategoriesChange = (next: CategoryKey[]) => { setCategories(next); if (place) void runScan(place, next); };
  ...
  const visibleResults = useMemo(() => {
    ...
    if (presenceFilter === "opportunity") list = list.filter((r) => r.level !== "full");
    if (presenceFilter === "zero") list = list.filter((r) => r.level === "zero");
    if (presenceFilter === "weak") list = list.filter((r) => r.level === "weak");
    if (presenceFilter === "full") list = list.filter((r) => r.level === "full");
  ...
  return (
    <div className="flex h-screen flex-col overflow-hidden bg-background text-foreground">
      <header className="relative z-[1100] flex shrink-0 items-center gap-2 border-b border-border bg-card/80 px-3 py-2 backdrop-blur sm:gap-3">
      ...
      <div className="relative z-0 flex min-h-0 flex-1 flex-col gap-0 p-0 lg:flex-row lg:gap-3 lg:p-3">
        <aside className="relative z-[20] flex h-1/2 min-h-0 w-full flex-col overflow-hidden border-b border-border bg-card/40 lg:h-auto lg:w-[460px] lg:shrink-0 lg:rounded-xl lg:border">
          <div className="flex shrink-0 items-center justify-between border-b border-border/60 px-4 py-2.5"><h2 className="text-sm font-semibold">Resultados</h2><span className="text-[11px] text-muted-foreground">{visibleResults.length} de {results.length}</span></div>
          <div className="min-h-0 flex-1 overflow-y-auto">
            {scanning ? <div className="space-y-2 px-4 py-3">{Array.from({ length: 6 }).map((_, i) => <div key={i} className="h-16 animate-pulse rounded-md border border-border/50 bg-muted/40" />)}</div>
              : error ? <p className="px-4 py-6 text-xs text-destructive">{error}</p>
              : visibleResults.length === 0 ? <p className="px-4 py-6 text-xs text-muted-foreground">{results.length === 0 ? "Busque uma cidade, bairro ou rua na barra de pesquisa para varrer a área." : "Nenhum estabelecimento contatável (WhatsApp ou Instagram) passou nos filtros atuais."}</p>
        ...
        <main className="relative z-0 min-h-0 flex-1 overflow-hidden border-t border-border lg:rounded-xl lg:border lg:shadow-lg">
          ...
          {scanning && <div className="absolute left-1/2 top-3 z-[500] flex -translate-x-1/2 items-center gap-2 rounded-full border border-border bg-card/95 px-3 py-1.5 text-[11px] text-muted-foreground shadow-lg"><Loader2 className="h-3.5 w-3.5 animate-spin text-primary" />Varrendo a área...</div>}
        </main>
      </div>
    </div>
  );
