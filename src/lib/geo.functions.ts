...
export const searchPlacesServer = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }): Promise<PlaceSuggestion[]> => {
    const q = data.q.trim(); if (q.length < 3) return [];
    ...
    return results.map((r) => { const bb = r.boundingbox; const parts = r.display_name.split(",").map((p) => p.trim()); return { label: r.display_name, shortLabel: parts.slice(0, 3).join(", "), lat: Number.parseFloat(r.lat), lon: Number.parseFloat(r.lon), boundingBox: bb && bb.length >= 4 ? { south: Number.parseFloat(bb[0] ?? "0"), north: Number.parseFloat(bb[1] ?? "0"), west: Number.parseFloat(bb[2] ?? "0"), east: Number.parseFloat(bb[3] ?? "0") } : null }; });
  });
  ...
  .handler(async ({ data }): Promise<{ elements: OverpassElement[] }> => {
    ...
    for (const mirror of OVERPASS_MIRRORS) {
      try {
        ...
        const json = (await response.json()) as { elements?: OverpassElement[] }; return { elements: json.elements ?? [] };
...
export const verifyLeadsServer = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }): Promise<{ leads: (Establishment & { verification: LeadVerification })[]; external: boolean }> => {
    if (!externalVerificationConfigured()) return { leads: data.leads.map((lead) => ({ ...lead, verification: { status: "unverified" as const, score: 50, reasons: ["Verificação externa não configurada."], checked: false, foundDigitalPresence: false, contactConfidence: lead.contact.whatsappValid ? "high" : lead.contact.phoneDigits ? "medium" : "low" } })), external: false };
    const candidates = data.leads.slice(0, 40);
    const verified = await verifyLeads(candidates);
    return { leads: verified, external: true };
  });
