...
function hashId(value: string): number { let hash = 2166136261; for (let i = 0; i < value.length; i += 1) { hash ^= value.charCodeAt(i); hash = Math.imul(hash, 16777619); } return Math.abs(hash >>> 0) || 1; }
function firstUrl(value: unknown): string { return Array.isArray(value) ? clean(value[0]) : clean(value); }
...
function normalizeArea(area: BoundingBox): BoundingBox | null {
  const south = Number(area.south), north = Number(area.north), west = Number(area.west), east = Number(area.east);
  if (![south, north, west, east].every(Number.isFinite)) return null;
  if (south < -90 || north > 90 || west < -180 || east > 180 || south > north || west > east) return null;
  return { south, north, west, east };
}
function areaSizeDeg2(area: BoundingBox): number { return Math.abs((area.north - area.south) * (area.east - area.west)); }
...
export async function queryOverturePlaces(area: BoundingBox): Promise<OverpassElement[]> {
  ...
  try {
    ...
    const sql = `SELECT CAST(id AS VARCHAR) AS id, names.primary AS name, basic_category, taxonomy.primary AS taxonomy_primary, taxonomy.hierarchy AS taxonomy_hierarchy, bbox.ymin AS latitude, bbox.xmin AS longitude, websites[1] AS website, socials[1] AS social, emails[1] AS email, phones[1] AS phone, addresses[1].freeform AS address, addresses[1].locality AS locality, addresses[1].region AS region, addresses[1].postcode AS postcode, operating_status, confidence, brand.names.primary AS brand FROM read_parquet('${path}', hive_partitioning=1) WHERE bbox.xmax >= ${west} AND bbox.xmin <= ${east} AND bbox.ymax >= ${south} AND bbox.ymin <= ${north} AND (operating_status IS NULL OR operating_status <> 'permanently_closed') AND (confidence IS NULL OR confidence > 0.2) LIMIT ${MAX_RESULTS};`;
    const reader = await connection.runAndReadAll(sql);
    const rows = reader.getRowObjectsJson() as OvertureRow[];
    return rows.filter((row) => clean(row.name) && Number.isFinite(Number(row.latitude)) && Number.isFinite(Number(row.longitude))).map((row) => {
      ...
      const website = firstUrl(row.website), social = firstUrl(row.social), email = firstUrl(row.email), phone = firstUrl(row.phone);
