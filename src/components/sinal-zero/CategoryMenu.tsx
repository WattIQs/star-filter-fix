...
export function CategoryMenu({ value, onChange, onScan, scanning = false }: CategoryMenuProps) {
  ...
  return <div className="flex shrink-0 items-center gap-1.5">
    <Button
      type="button"
      onClick={onScan}
      disabled={scanning || value.length === 0 || !onScan}
    ...
    <Popover>
      ...
      <PopoverContent align="start" sideOffset={8} collisionPadding={12} className="z-[5000] w-64 border-border bg-card p-2 shadow-2xl">
        <div className="mb-2 flex items-center justify-between px-1">
          <span className="text-[11px] font-semibold uppercase tracking-wide text-muted-foreground">Categorias</span>
          <button type="button" onClick={() => onChange(value.length === ALL_KEYS.length ? [] : ALL_KEYS)} className="text-[11px] text-primary hover:underline">
        ...
        <div className="max-h-64 space-y-0.5 overflow-y-auto pr-1">
          {ALL_KEYS.map((key) => {
            ...
            return <button key={key} type="button" onClick={() => toggle(key)} className={cn("flex w-full items-center justify-between rounded-md px-2 py-1.5 text-left text-xs transition-colors", active ? "bg-primary/10 text-primary" : "text-muted-foreground hover:bg-muted")}>
              {CATEGORY_LABELS[key]}
              {active && <Check className="h-3.5 w-3.5" />}
            </button>;
          })}
        </div>
        <p className="mt-2 border-t border-border pt-2 text-center text-[9px] text-muted-foreground">Selecione uma ou mais categorias e depois use Varrer área.</p>
      </PopoverContent>
    </Popover>
  </div>;
}
