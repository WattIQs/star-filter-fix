...
export function FiltersMenu({ ratingFilter, onRatingFilterChange, priceFilter, onPriceFilterChange, sortKey, onSortKeyChange }: FiltersMenuProps) {
  ...
  return <Popover>
    ...
    <PopoverContent align="end" sideOffset={8} collisionPadding={12} className="z-[5000] w-[300px] space-y-4 border-border bg-card p-4 shadow-2xl">
      <div className="flex items-center justify-between border-b border-border pb-3"><div><h3 className="text-sm font-semibold text-foreground">Filtros</h3><p className="mt-0.5 text-[10px] text-muted-foreground">Aplicados aos Sinais Zero encontrados</p></div>{activeCount > 0 && <button type="button" onClick={() => { onRatingFilterChange("any"); onPriceFilterChange("any"); onSortKeyChange("relevance"); }} className="text-[11px] font-medium text-primary hover:underline">Limpar</button>}</div>
      ...
      <div className="rounded-md border border-border/70 bg-muted/30 px-3 py-2"><p className="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">Presença digital</p><p className="mt-1 text-xs font-medium">Sinal Zero · obrigatório</p><p className="mt-0.5 text-[10px] text-muted-foreground">A lista principal já está restrita a oportunidades Sinal Zero.</p></div>
