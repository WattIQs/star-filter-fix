...
export function FiltersMenu({ ratingFilter, onRatingFilterChange, priceFilter, onPriceFilterChange, signalZeroOnly, onSignalZeroOnlyChange, sortKey, onSortKeyChange }: FiltersMenuProps) {
  const activeCount = (ratingFilter !== "any" ? 1 : 0) + (priceFilter !== "any" ? 1 : 0) + (signalZeroOnly ? 1 : 0) + (sortKey !== "relevance" ? 1 : 0);
  return <Popover>
    <PopoverTrigger asChild><Button variant="outline" size="sm" className="relative z-[1201] shrink-0 gap-1.5 border-border bg-background text-xs shadow-sm hover:bg-accent"><SlidersHorizontal className="h-3.5 w-3.5" /><span>Filtros</span>{activeCount > 0 && <span className="rounded-full bg-primary px-1.5 text-[10px] font-semibold text-primary-foreground">{activeCount}</span>}<ChevronDown className="h-3.5 w-3.5 opacity-70" /></Button></PopoverTrigger>
    <PopoverContent align="end" sideOffset={8} collisionPadding={12} className="z-[5000] w-[300px] space-y-4 border-border bg-card p-4 shadow-2xl">
      <div className="flex items-center justify-between border-b border-border pb-3"><div><h3 className="text-sm font-semibold text-foreground">Filtros</h3><p className="mt-0.5 text-[10px] text-muted-foreground">Refine os leads encontrados</p></div>{activeCount > 0 && <button type="button" onClick={() => { onRatingFilterChange("any"); onPriceFilterChange("any"); onSignalZeroOnlyChange(false); onSortKeyChange("relevance"); }} className="text-[11px] font-medium text-primary hover:underline">Limpar</button>}</div>
      <label className="flex cursor-pointer items-start gap-3 rounded-md border border-border/70 bg-muted/30 px-3 py-3 transition-colors hover:bg-muted/50"><input type="checkbox" checked={signalZeroOnly} onChange={(event) => onSignalZeroOnlyChange(event.target.checked)} className="mt-0.5 h-4 w-4 cursor-pointer accent-primary" /><span><span className="block text-xs font-semibold text-foreground">Somente Sinal Zero</span><span className="mt-0.5 block text-[10px] leading-relaxed text-muted-foreground">Mostra apenas negócios sem sinais de presença digital comercial.</span></span></label>
