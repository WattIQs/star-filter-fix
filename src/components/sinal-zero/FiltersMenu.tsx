  ...
  return (
    <label className="block space-y-1.5">
      ...
      <select
        ...
        className="h-10 w-full cursor-pointer appearance-none rounded-md border border-border bg-background px-3 text-xs text-foreground outline-none transition-colors hover:border-primary/50 focus:border-primary focus:ring-2 focus:ring-primary/20"
      >
        {options.map((option) => (
  ...
  return (
    <Popover>
      ...
      <PopoverContent
        ...
        sideOffset={8}
        collisionPadding={12}
        className="z-[5000] w-[300px] space-y-4 border-border bg-card p-4 shadow-2xl"
      >
        <div className="flex items-center justify-between border-b border-border pb-3">
          <div>
            <h3 className="text-sm font-semibold text-foreground">Filtros</h3>
            <p className="mt-0.5 text-[10px] text-muted-foreground">Aplicados instantaneamente aos resultados</p>
          </div>
          {activeCount > 0 && (
            <button
              type="button"
        ...
        <FilterSelect
          ...
          options={[
            { value: "any", label: "Qualquer classificação" },
        ...
        <FilterSelect
          ...
          options={[
            { value: "any", label: "Todos os níveis" },
            { value: "opportunity", label: "Pouca presença (zero ou fraca)" },
            { value: "zero", label: "Sinal zero" },
            { value: "weak", label: "Presença fraca" },
            { value: "full", label: "Presença forte" },
          ]}
        />
