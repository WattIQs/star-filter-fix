...
export function PlaceSearchBar({ onPick, scanning, currentLabel }: PlaceSearchBarProps) {
  ...
  const [value, setValue] = useState(""); const [suggestions, setSuggestions] = useState<SearchSuggestion[]>([]); const [open, setOpen] = useState(false); const [loading, setLoading] = useState(false); const [highlight, setHighlight] = useState(0);
  const boxRef = useRef<HTMLDivElement>(null); const requestIdRef = useRef(0); const pickedLabelRef = useRef<string | null>(null);
  useEffect(() => {
    ...
    const id = ++requestIdRef.current; let cancelled = false; setLoading(true);
    const timer = setTimeout(async () => {
      ...
      finally { if (!cancelled && id === requestIdRef.current) setLoading(false); }
    }, 220);
  ...
  const onKeyDown = (event: React.KeyboardEvent<HTMLInputElement>) => { if (!open || !suggestions.length) { if (event.key === "Escape") setOpen(false); return; } if (event.key === "ArrowDown") { event.preventDefault(); setHighlight((n) => (n + 1) % suggestions.length); } else if (event.key === "ArrowUp") { event.preventDefault(); setHighlight((n) => (n - 1 + suggestions.length) % suggestions.length); } else if (event.key === "Enter") { event.preventDefault(); const item = suggestions[highlight]; if (item) void pick(item); } else if (event.key === "Escape") setOpen(false); };
  const showSpinner = loading || scanning;
  return <div ref={boxRef} className="relative min-w-0 flex-1">
    <span className="pointer-events-none absolute left-3 top-1/2 z-10 flex h-4 w-4 -translate-y-1/2 items-center justify-center text-muted-foreground">{showSpinner ? <LoaderCircle className="h-4 w-4 animate-spin text-primary" aria-hidden="true" /> : <Search className="h-3.5 w-3.5" />}</span>
