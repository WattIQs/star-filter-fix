...
const AVATAR_COLORS = [
  "#06b6d4", "#f59e0b", "#8b5cf6", "#10b981", "#f43f5e", "#3b82f6", "#14b8a6", "#f97316",
];
  ...
  return (
    <>
      ...
      <div ref={panelRef} data-profile-menu className="fixed right-3 top-2 z-[9999] lg:right-3 lg:top-2">
          ...
          <div className="profile-menu-enter absolute right-0 top-12 w-[min(92vw,330px)] origin-top-right overflow-hidden rounded-2xl border border-border/80 bg-card/95 shadow-2xl shadow-black/30 backdrop-blur-2xl">
            <div className="relative overflow-hidden border-b border-border/60 p-4">
              ...
              <div className="relative flex items-center gap-3">
                <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-xl text-lg font-bold text-white shadow-lg" style={{ backgroundColor: color }}>{initial}</div>
                <div className="min-w-0 flex-1"><p className="truncate text-base font-bold text-foreground">{displayName || "Usuário"}</p><p className="text-[11px] text-muted-foreground">Seu perfil</p></div>
            ...
            <div className="p-2">
              <button type="button" onClick={() => setSettingsOpen(true)} className="flex w-full items-center gap-3 rounded-xl px-3 py-3 text-left text-sm text-muted-foreground transition-all hover:bg-muted hover:text-foreground"><Settings className="h-4 w-4" />Configurações</button>
            </div>
        ...
        {settingsOpen && (
          <div className="profile-menu-enter absolute right-0 top-12 w-[min(92vw,390px)] origin-top-right overflow-hidden rounded-2xl border border-border/80 bg-card/95 shadow-2xl shadow-black/30 backdrop-blur-2xl">
            <div className="flex items-center justify-between border-b border-border/60 px-4 py-3.5"><div><p className="text-sm font-bold text-foreground">Configurações</p><p className="text-[10px] text-muted-foreground">Personalize seu Star Filter</p></div><button type="button" onClick={() => setSettingsOpen(false)} className="rounded-lg p-1.5 text-muted-foreground hover:bg-muted hover:text-foreground" aria-label="Fechar configurações"><X className="h-4 w-4" /></button></div>
            <div className="space-y-3 p-3">
              <section className="rounded-xl border border-border/70 bg-background/35 p-3">
                <div className="mb-2 flex items-center gap-2"><UserRound className="h-4 w-4 text-primary" /><p className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Perfil</p></div>
                {editingName ? <div className="flex gap-2"><input autoFocus value={draftName} maxLength={60} onChange={(event) => setDraftName(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") void saveName(); }} placeholder="Como devemos te chamar?" className="h-10 min-w-0 flex-1 rounded-lg border border-border bg-background/70 px-3 text-sm outline-none transition focus:border-primary focus:ring-4 focus:ring-primary/10" /><button type="button" disabled={savingName} onClick={() => void saveName()} className="rounded-lg bg-primary px-3 text-primary-foreground transition hover:brightness-105 disabled:opacity-60" aria-label="Salvar nome">{savingName ? <span className="app-spinner h-3.5 w-3.5 border-primary-foreground/25 border-t-primary-foreground" /> : <Save className="h-4 w-4" />}</button></div> : <div className="flex items-center justify-between gap-3"><div><p className="text-sm font-semibold text-foreground">{displayName || "Usuário"}</p><p className="mt-0.5 text-[10px] text-muted-foreground">Nome exibido no seu perfil</p></div><button type="button" onClick={() => setEditingName(true)} className="rounded-lg p-2 text-muted-foreground hover:bg-muted hover:text-foreground" aria-label="Alterar nome"><Pencil className="h-4 w-4" /></button></div>}
              </section>
              <section className="rounded-xl border border-border/70 bg-background/35 p-3"><div className="mb-2 flex items-center gap-2"><div className="rounded-lg bg-primary/10 p-2 text-primary">{theme === "dark" ? <Moon className="h-4 w-4" /> : <Sun className="h-4 w-4" />}</div><div><p className="text-sm font-semibold text-foreground">Aparência</p><p className="text-[10px] text-muted-foreground">Escolha o modo de visualização</p></div></div><div className="grid grid-cols-2 gap-1.5 rounded-lg bg-muted/60 p-1"><button type="button" onClick={() => setSelectedTheme("light")} className={`flex items-center justify-center gap-2 rounded-md py-2 text-xs font-semibold transition-all ${theme === "light" ? "bg-background text-foreground shadow-sm" : "text-muted-foreground hover:text-foreground"}`}><Sun className="h-3.5 w-3.5" />Claro</button><button type="button" onClick={() => setSelectedTheme("dark")} className={`flex items-center justify-center gap-2 rounded-md py-2 text-xs font-semibold transition-all ${theme === "dark" ? "bg-background text-foreground shadow-sm" : "text-muted-foreground hover:text-foreground"}`}><Moon className="h-3.5 w-3.5" />Escuro</button></div></section>
            </div>
          </div>
        )}
      </div>
