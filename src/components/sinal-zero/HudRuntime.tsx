...
export function HudRuntime() {
  useEffect(() => {
    ...
    style.textContent = `
      .loading-state-enter{position:relative!important;display:flex!important;align-items:center!important;justify-content:center!important;min-height:250px!important;width:100%!important;margin:12px 0!important;padding:32px 20px 28px!important;box-sizing:border-box!important;overflow:hidden!important;text-align:center!important}
      .loading-state-enter::before{content:"";position:absolute;left:50%;top:50%;width:170px;height:170px;border:1px solid color-mix(in oklab,var(--color-primary) 10%,transparent);border-radius:50%;transform:translate(-50%,-50%);pointer-events:none;animation:sz-target-aura 2.8s ease-in-out infinite}
      .loading-state-enter::after{content:"";position:absolute;left:50%;top:50%;width:118px;height:118px;border:1px solid color-mix(in oklab,var(--color-cyan) 8%,transparent);border-radius:50%;transform:translate(-50%,-50%);pointer-events:none;animation:sz-target-aura 2.8s .8s ease-in-out infinite}
      ...
      @keyframes sz-target-materialize{0%{opacity:0;transform:scale(.58) rotate(-28deg);filter:blur(7px)}16%{opacity:1;transform:scale(1.02) rotate(0);filter:blur(0)}42%{opacity:1;transform:scale(1) rotate(125deg);filter:blur(0)}68%{opacity:.94;transform:scale(1.04) rotate(245deg);filter:blur(0)}86%{opacity:.82;transform:scale(.92) rotate(325deg);filter:blur(1px)}100%{opacity:0;transform:scale(.58) rotate(360deg);filter:blur(7px)}}
      ...
      @media(prefers-reduced-motion:reduce){.loading-state-enter::before,.loading-state-enter::after,.loading-state-spinner{animation:none!important}.loading-state-spinner{opacity:1!important;transform:none!important;filter:none!important}}
    `;
