...
export function HudRuntime() {
  useEffect(() => {
    ...
    style.textContent = `
      ...
      .loading-state-enter::before,.loading-state-enter::after{content:"";position:absolute;left:50%;top:50%;width:170px;height:170px;aspect-ratio:1/1;border:1px solid color-mix(in oklab,var(--color-primary) 10%,transparent);border-radius:50%;transform:translate(-50%,-50%);pointer-events:none}
      .loading-state-enter::before{animation:sz-target-aura 2.8s ease-in-out infinite}
      .loading-state-enter::after{width:118px;height:118px;border-color:color-mix(in oklab,var(--color-cyan) 8%,transparent);animation:sz-target-aura 2.8s .8s ease-in-out infinite}
      ...
      @keyframes sz-target-spin-shatter{0%,18%{opacity:1;transform:rotate(0deg) scale(1);filter:none}30%{opacity:.98;transform:rotate(108deg) scale(1);filter:blur(.1px)}40%{opacity:.72;transform:rotate(180deg) scale(.995);filter:blur(.7px)}48%{opacity:.28;transform:rotate(225deg) scale(.99);filter:blur(1.4px)}56%{opacity:.72;transform:rotate(270deg) scale(.995);filter:blur(.7px)}66%{opacity:.98;transform:rotate(306deg) scale(1);filter:blur(.1px)}82%,100%{opacity:1;transform:rotate(360deg) scale(1);filter:none}}
