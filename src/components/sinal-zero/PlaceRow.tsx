...
interface PlaceRowProps { place: Establishment; active: boolean; saved: boolean; onSelect: (id: string) => void; onToggleSave: (place: Establishment) => void; }
function Stars({ rating }: { rating: number }) { return <span className="flex items-center gap-1 text-xs"><span className="font-semibold text-signal-weak">{rating.toFixed(1)}</span><span className="flex">{[1,2,3,4,5].map((i)=><Star key={i} className={cn("h-3 w-3",i<=Math.round(rating)?"fill-signal-weak text-signal-weak":"text-muted-foreground/40")} />)}</span></span>; }
export function PlaceRow({ place, active, saved, onSelect, onToggleSave }: PlaceRowProps) {
  const { contact, details } = place;
  return <article onClick={()=>onSelect(place.id)} className={cn("group cursor-pointer border-b border-border/60 px-4 py-3 opacity-0 [animation:lead-enter_360ms_ease-out_forwards] transition-all duration-200","hover:-translate-y-px hover:bg-accent/40 hover:shadow-[inset_3px_0_0_hsl(var(--primary))]",active&&"translate-x-0.5 bg-primary/10 shadow-[inset_3px_0_0_hsl(var(--primary))]")}>
    <div className="flex items-start gap-3"><div className="min-w-0 flex-1">
      ...
      <div className="mt-0.5 flex flex-wrap items-center gap-x-2 gap-y-0.5 text-xs text-muted-foreground">{place.rating!==null?<Stars rating={place.rating}/>:<span className="text-muted-foreground/60">Sem nota pública</span>}<span>·</span><span>{place.category}</span>{place.priceLevel&&<><span>·</span><span className="text-signal-full">{"$".repeat(place.priceLevel)}</span></>}</div>
    ...
    </div><div className="flex shrink-0 flex-col items-end gap-2"><SignalBadge level={place.level}/><button type="button" aria-label={saved?"Remover dos salvos":"Salvar lead"} onClick={(e)=>{e.stopPropagation();onToggleSave(place);}} className="rounded-full border border-border p-1.5 text-muted-foreground transition-all hover:scale-105 hover:bg-accent hover:text-foreground">{saved?<BookmarkCheck className="h-3.5 w-3.5 text-primary"/>:<Bookmark className="h-3.5 w-3.5"/>}</button></div></div>
  </article>;
}
